...
| 正本 | 用途 |
|---|---|
| `00_note制作・公開システム.md` | noteの制作、Marketing Review、Header Production、Publication Dry Run／Transaction、公開後検証、再開、復旧を接続するSOP |
...
実データの作成・更新はProduction／Repository Integrationが担い、Publication PackageのG5承認と外部公開操作のHuman Publication Approval、価格、自己開示はHuman Owner／Approverが担う。Publication Decision SummaryはTransaction時のPublication Settings再構成に使うCanonical Inputであり、設定が下書きへ永続化されることを前提にしない。配置、Archive、CHANGELOG、Gitは `REPOSITORY_RULES.md` に従う。
