...
**Status:** Current / Operational v1.10 / Marketing Review β
**責任:** noteの企画・制作・Marketing Review・公開準備・公開後記録およびSession単位のSNS展開を、既存AI Production Pipelineへ接続する媒体別運用
...
| 正本 | 用途 |
|---|---|
| `00_note制作・公開システム.md` | noteの制作、Marketing Review β、公開、再開、復旧を接続するSOP |
...
| `10_Section制作台本テンプレート.md` | 意味づけ・企画で採用されたSectionの制作台本、稿状態、Marketing Review／Publication Decision記録テンプレート |
| `11_公開成果物記録テンプレート.md` | Marketing／Human承認済みDecisionと公開済み最終稿・公開事実の記録テンプレート |
...
Marketing ReviewはSection Statusを増やさず、`Marketing Input Pending`／`Marketing Revision Required`／`Marketing Approved`／`Human Decision Required`のsubstatusとしてSection制作台本へ記録する。Marketingは内容完成稿である第2稿から開始し、本文を直接修正せずRequirementとPublication Decisionを返す。第2稿、Human完遂Reviewまたは必要な実素材が不足する場合は開始しない。Human Final Approval済み最終稿をnote下書きへ反映し公開ボタン直前で停止したβ状態は`Publication Prepared / Not Published`であり、`Published`またはG8 PASSにしない。
